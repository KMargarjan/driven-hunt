...
| 11 | Run Task 9 (the four-agent workflow) through the loop itself, fix the findings, and write the new merge policy into CLAUDE.md | in progress | Branch `task-9-agent-workflow` (PR #4), merged up from `main`. **Scope is the Director's**, transcribed verbatim in `ESCALATE.md` ("Task 11's dispatch, verbatim"): the review of Task 9 *and* the merge-policy and `NEEDS KAREN` edits in one task. Merge policy: the Director retargets and merges after a clean-tree harness PASS on the code commit, Reviewer `PASS`, green CI and paperwork-only commits after it; Karen no longer merges; the Builder still never merges |
