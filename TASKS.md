...
| 34 | **ROADMAP 1.6 (harness):** run the specs with 2 players | **reviewed: PASS, 2 rounds** (notes queued as row 34a). NEEDS KAREN to exercise the half after the click | Branch `task-34-harness-2p`, from `main` (`2545891`). `python tools/studio_mcp.py test2` runs the SAME gate, runners and specs as `test`, read from three Studio instances instead of one; `test` is untouched and stays the default, and no spec knows which mode it is in. **StudioMCP cannot start the test** - `start_stop_play` takes `is_start` and `studio_id` and nothing else - so the mode prints the exact clicks, waits up to 180 s for three NEW studios, and claims nothing if nobody presses Start. The token is written FIRST, because a local test copies the place as it stands when Start is pressed and the runners refuse a token older than 120 s. It then identifies the instances by identity (the listing from before the click), classifies them with `get_studio_state` (Server vs Client), asks each client which TEAM its player is on, replays the input scenarios into the **shooter's** client, reads all three reports, checks the server and the shooter, and prints the driver's report as an observation - a driver carries no gun, so its weapon specs cannot pass. `tests/server/match_teams.spec.luau` is written to be true for whatever number of players is present, so it runs in both modes. Harness `PASS: 27/27 @ 8336f4d (clean tree)`; 229 server and 58 client `it` blocks. **Round 1 found three defects in the unexercised half** and all three were real: the wait was for three studios in total rather than three new ones; the replay target was picked by list order, which has nothing to do with who got the gun; and `match_live.spec` asserted a one-player fact inside the suite the 2-player run requires to be green. `ESCALATE.md` carries the **NEEDS KAREN** entry: one command, four clicks, and the half after the click is exercised |
| 34a | **Task 34's review notes** (non-blocking) | todo | From `reviews/task-34/RESULT.md` (round 2, PASS). Round 1's three blocking findings were fixed in round 2; these are what is left, and **eight of the eleven are in the half of `run_test2` nobody has exercised yet**, so the run Karen's click unlocks should come first. **(a)** the docstring's step 7a.2 still says the replay goes to the FIRST client named by its `studio_id`, which the code and the `Two players` section now contradict - it is the shooter's client. **(b)** the docstring's `Two players` item 5 names "client 1" and omits the `One client is on the Shooters team` check, so claim 10's "every check is now there" overstates by one. **(c)** the report loop's `json.loads` and `console()` sit in a `try` whose only `finally` clears the token, so an exception there returns through `main` WITHOUT `end_session` - claim 8's "every exit path stops the test instances" is false until the body is wrapped in `try/finally: end_session(...)`. Round 1 raised this for the classify path; that one was fixed and this one was not. **(d)** the advertised window and the waited window disagree: `START_CLICKS` promises 120 s (printed after up to 15 s of token-sync wait) while `run_test2` accepts a Start up to 180 s later, and `TestKit.TOKEN_MAX_AGE` is 120 - a Start at ~150 s gives three refused runners, ~360 s of waiting and three report FAILs with no line saying the click was late. Cap the wait at the token's remaining life, or say "the token expired - run it again". **(e)** the two team queries are sequential (45 s each) and the stage inside `replay_input` may add 60 s, while `input_driving.spec`'s `ARRIVE_TIMEOUT = 25` starts counting a few seconds after Start - in `run_test` the replay begins ~2 s after Play, here it can begin a minute later and fail the shooter for a reason that has nothing to do with two players. Query both clients concurrently, or bound the team wait. **(f)** the spec-name comparison hardcodes `"ServerStorage.Tests." + basename`, where `run_test` derives the path from the Rojo sourcemap (`file_to_path`); a spec in a subfolder of `tests/server/` would be reported as both "not run" and "not in repo". **(g)** there is no equivalent of `run_test`'s final `Gate closed afterwards` check - the token is cleared, but nothing confirms Studio saw it, which is the check that stops Karen's next playtest running the suite. **(h)** `wait_for` returns `None` when its last call raised, so `len(found)` can raise `TypeError` and replace the `NEEDS KAREN: nobody pressed Start` line with a harness-fault line - `len(found or [])`. **(i)** `match_teams.spec`'s `it("splits two players into one driver and one shooter...")` is the only name the one-player harness prints, and in that session the body asserts the opposite thing; name it for the invariant it holds at every size. **(j)** `match_live.spec`'s rewritten `put every player on a team` is a bare loop, so with zero players it passes having asserted nothing - one `expect(#Players:GetPlayers() >= 1)` inside that `it` closes it. **(k)** `match_client.spec` asserts the HUD says `SHOOTER`, which is a second reason the driver's client can only ever be an observation (alongside the missing gun); say so where the driver's report is printed |
