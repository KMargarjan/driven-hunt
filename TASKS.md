...
| 36a | **Task 36's leftovers: the marker the second client never receives, and the review notes** | todo | **(1) MEASURED TWICE AND UNEXPLAINED.** In a Studio "Clients and Servers" local test the server marks BOTH players (`weapon_hits.spec`'s own note names them, and its counter asserts the two `FireClient` calls happened), and the SECOND client receives nothing -- 0 markers in 174 s -- while every `FireAllClients` payload in the same session (`MatchState`, `MatchEvent`) reaches it. So `hit_marker.spec`'s "a marker sent by the server arrives here" is asserted for the SHOOTER only, and the driver asserts the remote, the Hud's connection and the drawing path instead; both sides now note what they sent and saw with a timestamp, so the next run says whether the send was even inside the window. Worth settling before release: every hit marker in the real game is a targeted `FireClient`. **(2)** the review notes, from `reviews/task-36/RESULT.md` |
